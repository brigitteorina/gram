...
function Card({ users }) {
  function patchLikes(e, userId, postId, user) {
    addLikes(`https://my-json-server.typicode.com/brigitteorina/My-json-file/users/${userId}`).then((resp) => {
      console.log(resp);
  ...
  function handleComments(userId, postId, commentDiv, user) {
    ...
    addComments(`https://my-json-server.typicode.com/brigitteorina/My-json-file/users/${userId}`).then((resp) => {
      // eslint-disable-next-line
      user.postAddress.map((requiredPost) => {
